...
原则：**命名越准，协作越快。** 会叫名字，模型才进得对知识区；只会形容感觉，就容易猜谜和返工。
...
- **Meta-prompting**：先让模型写「下一问该用哪些总称」  
- **Prompt chaining**：词表 → 概念图 → 任务，分步跑  
- **Role + register**：指定学术 / 工程 / 运维等语域  
- **Few-shot**：示例里坚持 preferred terms  
- **Output contract**：回复必须使用词表标准名  

这些不是替代词表，而是词表对齐之后的放大器。

一份填好的小样见 [examples/http-api.md](../examples/http-api.md)。
